...
const STATUS_OPTIONS = ["\u0110ang b\xE1n", "H\u1EBFt m\xF3n", "T\u1EA1m ng\u01B0ng"];
export default function FoodMenu() {
  ...
  const cycleStatus = (id, current) => {
    ...
    setStatuses((prev) => ({ ...prev, [id]: STATUS_OPTIONS[(idx + 1) % STATUS_OPTIONS.length] }));
  };
  return <div className="p-6 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-700" style={{ color: "var(--foreground)" }}>Món ăn</h2>
          <p className="text-xs mt-0.5" style={{ color: "var(--muted-foreground)" }}>{filtered.length} món</p>
        </div>
      </div>
  ...
  >
    ...
    return <div key={food.id} className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: "var(--border)" }}>
              <div className="relative h-40 bg-[var(--secondary)] overflow-hidden">
                <img src={food.image} alt={food.name} className="w-full h-full object-cover" />
                <span className="absolute top-2 right-2 text-xs px-2 py-0.5 rounded font-500" style={{ background: s.bg, color: s.text }}>
                  {status}
                </span>
              </div>
              <div className="p-3">
                <div className="text-sm font-600 leading-snug mb-1" style={{ color: "var(--foreground)" }}>{food.name}</div>
                <div className="text-xs mb-2" style={{ color: "var(--muted-foreground)" }}>{food.category}</div>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-700" style={{ color: "var(--primary)" }}>{formatCurrency(food.price)}</span>
                  <button
      ...
      style={{ borderColor: s.text, color: s.text, background: s.bg }}
