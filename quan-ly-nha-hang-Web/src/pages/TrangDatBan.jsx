      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="mb-8 flex items-center justify-center gap-2 overflow-x-auto">
          {CAC_BUOC.map((tenBuoc, index) => {
            ...
            return (
              <div key={tenBuoc} className="flex items-center gap-2">
                <div
                  ...
                  style={{
                    background:
                      soBuoc <= buoc
                        ? "rgba(200,136,42,.9)"
                        : "rgba(200,136,42,.08)",
                    color: soBuoc <= buoc ? "#1a120a" : "rgba(240,216,144,.4)",
                  }}
                >
                  {soBuoc}
                </div>
                <span className="hidden text-xs sm:inline">{tenBuoc}</span>
                {soBuoc < CAC_BUOC.length && (
        ...
        {buoc < 5 ? (
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
            <div>
              {buoc === 1 && (
                <BuocChon
                  chiNhanh={chiNhanh}
                  setChiNhanh={setChiNhanh}
                  ngay={ngay}
                  setNgay={setNgay}
                  soKhach={soKhach}
                  setSoKhach={setSoKhach}
                  khiTiepTuc={() => setBuoc(2)}
                />
              )}
              {buoc === 2 && (
                <BuocChonGio
                  ...
                  khiTiepTuc={() => setBuoc(3)}
                  khiQuayLai={() => setBuoc(1)}
                />
              )}
              {buoc === 3 && (
                <BuocMonAn
              ...
              {buoc === 4 && (
                <BuocThongTin
                  ...
                  setDichVuBoSung={setDichVuBoSung}
                  khiXacNhan={xacNhan}
                  khiQuayLai={() => setBuoc(3)}
                />
              )}
            </div>
            <TomTatDatBan
          ...
          <div className="mx-auto max-w-2xl">
            <div className="mb-6 text-center">
              ...
              >
                Mã đặt bàn:{" "}
                <b style={{ color: "rgba(232,184,75,.95)" }}>{maDatBan}</b>
              </p>
            </div>
            <div className="card-warm rounded-2xl p-5 sm:p-7">
              <div className="grid grid-cols-2 gap-5 text-sm">
                <div>
                  ...
                  {thongTinChiNhanh?.ten}
                </div>
                <div>
                  <span className="block text-xs opacity-40">Giờ</span>
                  {gioDaChon}
                </div>
                <div>
                  <span className="block text-xs opacity-40">Ngày</span>
                  {ngay}
                </div>
                <div>
                  <span className="block text-xs opacity-40">Số khách</span>
                  {soKhach}
                </div>
                <div>
                  <span className="block text-xs opacity-40">Người đặt</span>
                  {thongTin.hoTen}
                </div>
                <div>
                  <span className="block text-xs opacity-40">Điện thoại</span>
              ...
              >
                <div className="flex justify-between">
                  <span className="text-sm opacity-50">Món và dịch vụ</span>
                  <span>{tongTien.toLocaleString("vi-VN")}₫</span>
                </div>
                {thongTin.dip !== "khong" && (
                  <p className="mt-2 text-xs opacity-50">
                    Dịp:{" "}
                    {DIP_DAT_BAN.find((dip) => dip.id === thongTin.dip)?.ten}
                  </p>
                )}
              </div>
            </div>
