...
import { Badge, Button, Card, ConfirmDialog, EmptyState, Input, PageHeader, Pagination, Table, Td, Tr } from "../../components/admin/ui";
function SearchIcon() {
...
export default function Customers() {
  ...
  />
    ...
    setSearch(v);
    setPage(1);
  }} placeholder="Tên, SĐT, email..." icon={<SearchIcon />} />
        </div>
        <div style={{ marginTop: 12 }}>
          {paged.length === 0 ? <EmptyState /> : <Table headers={["H\u1ECD t\xEAn", "\u0110i\u1EC7n tho\u1EA1i", "Email", "L\u01B0\u1EE3t \u0111\u1EB7t", "T\u1ED5ng chi ti\xEAu", "Tr\u1EA1ng th\xE1i", "Thao t\xE1c"]}>
              {paged.map((c) => <Tr key={c.id}>
                  <Td>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#f5f0e8", display: "flex", alignItems: "center", justifyContent: "center", color: "#c9922a", fontWeight: 700, fontSize: 12 }}>
                        {c.name[0]}
                      </div>
                  ...
                  <Td mono>{c.email}</Td>
                  <Td>
                    <span style={{ fontWeight: 600, color: "#c9922a" }}>{c.bookings}</span>
                  </Td>
                  <Td>
                    <span style={{ fontWeight: 600, color: "#2c1a0e" }}>{c.totalSpent.toLocaleString("vi-VN")} đ</span>
                  </Td>
