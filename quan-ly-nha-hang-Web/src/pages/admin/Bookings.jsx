...
import { Badge, Button, Card, ConfirmDialog, EmptyState, PageHeader, Pagination, Select, Table, Td, Tr } from "../../components/admin/ui";
function TrashIcon() {
...
export default function Bookings() {
  ...
  const branchOptions = [
    ...
    ...Array.from(new Set(items.map((b) => b.branch))).map((b) => ({ label: b, value: b }))
  ];
  ...
  return <div style={{ padding: "28px 32px" }}>
      <PageHeader title="Quản lý đặt lịch" subtitle="Tất cả đặt lịch từ mọi chi nhánh" />
      <Card>
    ...
    { label: "\u0110\xE3 h\u1EE7y", value: "cancelled" }
        ...
        <div style={{ marginTop: 12 }}>
          {paged.length === 0 ? <EmptyState /> : <Table headers={["M\xE3 \u0110L", "Kh\xE1ch h\xE0ng", "Chi nh\xE1nh", "Th\u1EDDi gian", "S\u1ED1 kh\xE1ch", "Tr\u1EA1ng th\xE1i", "Thao t\xE1c"]}>
              {paged.map((b) => <Tr key={b.id}>
                  <Td mono>{b.id}</Td>
                  <Td><span style={{ fontWeight: 600 }}>{b.customer}</span></Td>
                  <Td>{b.branch}</Td>
                  <Td mono>{b.datetime}</Td>
                  <Td>
                    <span style={{ fontWeight: 600, color: "#5a4030" }}>{b.guests} người</span>
                  </Td>
                  <Td><Badge status={b.status} /></Td>
                  <Td>
                    <Button variant="danger" size="sm" icon={<TrashIcon />} onClick={() => setDeleteId(b.id)}>Hủy</Button>
                  </Td>
                </Tr>)}
            </Table>}
